...
`doorGroup` 변수명과 `applyKeyState()`의 `visible=!hasKey` 토글 구조는 참조가 많아
**그대로 뒀다.** 안에 들어가는 메시만 문 슬래브(`gDoorSlab`, 제거함)에서 사다리
레일+가로대(`gLadderRail`/`gLadderRung`)로 바꿨다. 자물쇠(`gLockBody`/`gLockShkl`,
`mLock`)는 기존 것을 그대로 재사용 — 가로대 사이에 걸린 모습으로 자연스럽게 붙는다.
새 재질도 새 광원도 없다(둘 다 기존 `mDoor`/`mLock` 재사용).

2층에는 이 사다리가 필요 없다(열쇠가 처음부터 `true`라 `doorGroup`이 즉시 숨겨진다).
