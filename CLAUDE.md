...
sw.js               Service Worker (캐시명 dungeon-escape-v23)
icon-192.svg        아이콘
icon-512.svg        아이콘
README.md
...
| 중간 | **정전 + 손전등** | 정전(천장등 세기 일괄 0)은 구조 변경 없이 가능. 손전등은 `SpotLight` 1개가 필요해 **광원 개수가 늘어나는 유일한 후보** — 실기기 검증 없이 진행 금지. 정전 구간 한정 사용을 권장(안 그러면 어두운 구역의 "몬스터가 안 보임" 설계가 무력화됨). 예산은 +1을 그대로 받거나 `panelLights`를 1씩 깎아 맞바꾸는 두 가지 길이 있음 |
